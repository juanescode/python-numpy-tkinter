...
def crear_ventana_metodo(titulo, labels, ejecutar_func):
    ...
    tk.Button(ventana, text="Calcular", command=lambda: ejecutar_func(entries)).grid(row=len(labels), column=0, columnspan=2, pady=10)
...
def crear_ventana_lagrange():
    ...
    tk.Label(ventana, text="Número de puntos n:").grid(row=0, column=0)
    ...
    tk.Button(ventana, text="Siguiente", command=siguiente).grid(row=1, column=0, columnspan=2, pady=10)
...
def crear_ventana_lagrange_valores(n):
    ...
    for i in range(n):
        tk.Label(ventana, text=f"x{i}:").grid(row=i, column=0, padx=10, pady=5, sticky="e")
        entry_x = tk.Entry(ventana, width=5)
        entry_x.grid(row=i, column=1, padx=10, pady=5, sticky="w")
        entries_x.append(entry_x)

        tk.Label(ventana, text=f"y{i}:").grid(row=i, column=2, padx=10, pady=5, sticky="e")
        entry_y = tk.Entry(ventana, width=5)
        entry_y.grid(row=i, column=3, padx=10, pady=5, sticky="w")
        entries_y.append(entry_y)

    tk.Label(ventana, text="xp:").grid(row=n, column=0, padx=10, pady=5, sticky="e")
    entry_xp = tk.Entry(ventana, width=5)
    ...
    tk.Button(ventana, text="Calcular", command=ejecutar).grid(row=n+1, column=0, columnspan=4, pady=10)

def crear_ventana_gauss_jacobi():
    ventana = tk.Toplevel(root)
    ventana.title("Método de Gauss-Jacobi - Seleccionar Tamaño de Matriz")

    tk.Label(ventana, text="Número de filas:").grid(row=0, column=0)
    entry_filas = tk.Entry(ventana)
    entry_filas.grid(row=0, column=1)

    tk.Label(ventana, text="Número de columnas:").grid(row=1, column=0)
    ...
    tk.Button(ventana, text="Siguiente", command=siguiente).grid(row=2, column=0, columnspan=2, pady=10)
...
def crear_ventana_gauss_jacobi_valores(filas, columnas):
    ...
    tk.Label(ventana, text="=").grid(row=0, column=columnas, rowspan=filas, padx=10, pady=5)
    ...
    for i in range(columnas):
        tk.Label(ventana, text=f"x{i}0:").grid(row=filas, column=i, padx=10, pady=5, sticky="e")
        entry = tk.Entry(ventana, width=5)
        entry.grid(row=filas+1, column=i, padx=10, pady=5, sticky="w")
        entries_x0.append(entry)

    tk.Label(ventana, text="tol:").grid(row=filas+2, column=0, padx=10, pady=5, sticky="e")
    entry_tol = tk.Entry(ventana)
    entry_tol.grid(row=filas+2, column=1, padx=10, pady=5, sticky="w")

    tk.Label(ventana, text="max_iter:").grid(row=filas+3, column=0, padx=10, pady=5, sticky="e")
    ...
    tk.Button(ventana, text="Calcular", command=ejecutar).grid(row=filas+4, column=0, columnspan=columnas+2, pady=10)
...
ttk.Button(root, text="Método de Bisección", command=lambda: crear_ventana_metodo(
    "Método de Bisección",
